  ...
  async GET(_req, _ctx) {
    ...
    if (maybeAccessToken) {
      ...
      if (userData) {
        ...
        if (search) {
          const searchResponse = await spotifyApi.search(_req, search, ["track", "artist"], 50);
          const searchData: SearchResult = await searchResponse.json();
          console.log('search', searchData);
          return _ctx.render({ userData, searchData });
        }
...
export default function Home({ url, data }: PageProps) {
  if (data) {
    ...
    const searchData: SearchResult = data.searchData;

    const images = searchData?.tracks.items.map(track => track.album.images.find(image => image)?.url);
      ...
      <div class="container">
        ...
        <div class="bar">
          ...
          <div class="input-container">
            <form action="#results">
              <input type="text" id="search" name="search" placeholder="Search album artwork" />
              {/* <button type="submit">Search</button> */}
            </form>
            <div class="input-icon">
              <svg role="img" height="24" width="24" class="Svg-sc-1bi12j5-0 EQkJl mOLTJ2mxkzHJj6Y9_na_" aria-hidden="true" viewBox="0 0 24 24"><path d="M10.533 1.279c-5.18 0-9.407 4.14-9.407 9.279s4.226 9.279 9.407 9.279c2.234 0 4.29-.77 5.907-2.058l4.353 4.353a1 1 0 101.414-1.414l-4.344-4.344a9.157 9.157 0 002.077-5.816c0-5.14-4.226-9.28-9.407-9.28zm-7.407 9.279c0-4.006 3.302-7.28 7.407-7.28s7.407 3.274 7.407 7.28-3.302 7.279-7.407 7.279-7.407-3.273-7.407-7.28z"></path></svg>
            </div>
          </div>
          <button class="profile-button">
            <div class="profile-picture" style={"background-image: url(" + userData.images.find(i => i).url + ");"}></div>
            <span>{userData.display_name}</span>
          </button>
        </div>
        <div class="artwork-list" id="results">
          {uniqueImages.map(img => <div class="artwork" style={`background-image: url(${img})`} />)}
        </div>
      </div>
    )
  }
